...
class ExtendGridEnv(discrete.DiscreteEnv):
    """
        Has the following members
        - nS: number of states
        - nA: number of actions
    ...
    def __init__(self):
        ...
        payoff = [[0, 0] for i in range(num_state)]
        ...
        for i in range(9):
            state = self.encode_state(i, i)
            payoff[state][0] = payoff[state][1] = 1
        state = self.encode_state(self.target0, self.target0)
        payoff[state][0] = payoff[state][1] = 100
        state = self.encode_state(self.target1, self.target1)
        payoff[state][0] = payoff[state][1] = 200
        ...
        for state in range(num_state):
            for action in range(num_action):
                ...
                [pos0, pos1] = self.decode_state(new_state)
                # if new_state == self.encode_state(self.target0, self.target1):
                if (pos0 == self.target0 and pos1 == self.target0) or (pos1 == self.target1 and pos0 == self.target1):
                    done = True
                else:
                    done = False
                P[state][action].append((1.0, new_state, reward, np.array([done] * 2)))
    ...
    def next_state(self, state, action0, action1):
        dir_x = [0, 0, -2, 2, 0]  # up, down, left, right
        dir_y = [1, -1, 0, 0, 0]  # up, down, left, right
        [pos0, pos1] = self.decode_state(state)
