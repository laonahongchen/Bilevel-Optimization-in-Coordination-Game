...
class MASampler(Sampler):
    ...
    def sample(self, explore=False):
        ...
        if explore:
            ...
            mix_observe_1 = tf.one_hot(self._current_observation_n[0], self.env.num_state)
    ...
    def sample(self, explore=False):
        ...
        if explore:
            ...
            mix_observe_1 = np.hstack((tf.one_hot(self._current_observation_n[1], self.env.num_state),
                                       tf.one_hot([action_n[0]], self.env.action_num)))
            supplied_observation.append(mix_observe_1)
            ...
            # print('observation:')
            # print(self._current_observation_n[0])
            ...
            # mix_observe_0 = np.array([mix_observe_0])
            action_0 = self.agents[0].act(mix_observe_0)
            # action_0 = action_0[0]
            # action_0 = np.array([0])
            supplied_observation.append(mix_observe_0)
            ...
            # print(self._current_observation_n[1], action_0)

            print(self._current_observation_n[1].shape)
            ...
            # mix_observe_1 = np.array([mix_observe_1])
        ...
        for i, agent in enumerate(self.agents):
            ...
            agent.replay_buffer.add_sample(
                ...
                terminal=np.float32(done_n[i]),
                # next_observation=next_observation_n[i].astype(np.float32),
                next_observation=tf.one_hot(next_observation_n[i], self.env.num_state),
                # opponent_action=opponent_action.astype(np.float32)
                opponent_action=tf.one_hot(opponent_action, self.env.action_num)
            )
...
class Bi_continuous_Sampler(Sampler):
    def __init__(self, agent_num, max_path_length=20, min_pool_size=10e4, batch_size=64, global_reward=False, **kwargs):
        # super(MASampler, self).__init__(**kwargs)
        self.agent_num = agent_num
        self._max_path_length = max_path_length
    ...
    def sample(self, explore=False):
        ...
        if explore:
            ...
            mix_observe_1 = np.hstack((self._current_observation_n[1], action_n[0]))
            # action_1 = self.agents[1].act(mix_observe_1)
...
class Continuous_MASampler(Sampler):
    def __init__(self, agent_num, max_path_length=20, min_pool_size=10e4, batch_size=64, global_reward=False, **kwargs):
        # super(MASampler, self).__init__(**kwargs)
        self.agent_num = agent_num
        self._max_path_length = max_path_length
