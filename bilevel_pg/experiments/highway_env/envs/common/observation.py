...
class KinematicObservations(ObservationType):
    ...
    def __init__(self, env, features=FEATURES, vehicles_count=4, **kwargs):
        """
        :param env: The environment to observe
        :param features: Names of features used in the observation
        :param vehicles_count: Number of observed vehicles
    ...
    def space(self):
        return [spaces.Box(shape=(len(self.features) * self.vehicles_count,), low=-1, high=1, dtype=np.float32) for _ in range(len(self.env.all_vehicles))]
    ...
    def get_observations(self):
        ...
        for vehicle in self.env.all_vehicles:
