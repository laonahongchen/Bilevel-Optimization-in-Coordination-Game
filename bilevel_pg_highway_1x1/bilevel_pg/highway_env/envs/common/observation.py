...
class KinematicObservations(ObservationType):
    """
        Observe the kinematics of nearby vehicles.
    """
    FEATURES = ['presence', 'x', 'y', 'vx', 'vy']

    def __init__(self, env, features=FEATURES, vehicles_count=2, **kwargs):
        """
        :param env: The environment to observe
        :param features: Names of features used in the observation
        :param vehicles_count: Number of observed vehicles
        ...
        self.features = features
        #self.vehicles_count = vehicles_count
        self.level_vehicles_count = vehicles_count
        self.vehicles_count = 5

    def space(self):
        return [spaces.Box(shape=(len(self.features) * self.vehicles_count,), low=-1, high=1, dtype=np.float32) for _ in range(len(self.env.road.vehicles))]

    def normalize(self, vehicle, df):
        """
            Normalize the observation values.
        ...
        side_lanes = self.env.road.network.all_side_lanes(vehicle.lane_index)
        x_position_range = 5.0 * MDPVehicle.SPEED_MAX
        y_position_range = AbstractLane.DEFAULT_WIDTH * len(side_lanes)
        velocity_range = 2*MDPVehicle.SPEED_MAX
        df['x'] = utils.remap(df['x'], [-x_position_range, x_position_range], [-1, 1])
    ...
    def get_observations(self):
        obs_n = []
        #print(len(self.env.all_vehicles))
        for vehicle in self.env.road.vehicles:
            obs = self.observe(vehicle)
            #print(vehicle, "observation", obs)
            obs_n.append(obs)
    ...
    def observe(self, vehicle):
        ...
        # Add nearby traffic
        close_leader_vehicles = self.env.road.closest_leader_vehicles_to(vehicle)
        close_follower_vehicles = self.env.road.closest_follower_vehicles_to(vehicle)

        #close_vehicles = self.env.road.closest_vehicles_to(vehicle, self.vehicles_count - 1)
        
        #print("-------TESTING--------")
        #print(vehicle, close_vehicles)
        
        if close_leader_vehicles:
            df = df.append(pandas.DataFrame.from_records(
                [v.to_dict(vehicle)
                for v in close_leader_vehicles[-self.level_vehicles_count + 1:]])[self.features],
                        ignore_index=True)
        if close_follower_vehicles:
            df = df.append(pandas.DataFrame.from_records(
                [v.to_dict(vehicle)
                for v in close_follower_vehicles[-self.level_vehicles_count + 1:]])[self.features],
                        ignore_index=True)    
        #print("df for a single agent: ", df)
        
        
        # Normalize
        df = self.normalize(vehicle, df)
        # Fill missing rows
        if df.shape[0] < self.vehicles_count:
            rows = -np.ones((self.vehicles_count - df.shape[0], len(self.features)))
            df = df.append(pandas.DataFrame(data=rows, columns=self.features), ignore_index=True)
...
def observation_factory(env, config):
    ...
    elif config["type"] == "Kinematics_MA":
        return KinematicObservations(env, **config)
    else:
