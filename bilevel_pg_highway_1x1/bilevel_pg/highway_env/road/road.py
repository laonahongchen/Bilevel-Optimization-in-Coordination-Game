...
class Road(Loggable):
    ...
    def closest_vehicles_to(self, vehicle):
        sorted_v = []
        


    def closest_leader_vehicles_to(self, vehicle, count = 2):
        sorted_v = sorted([v for v in self.vehicles
                           if v is not vehicle
                           and v.index < self.leader_num],
                          key=lambda v: abs(vehicle.lane_distance_to(v)))
        return sorted_v[:count]

    def closest_follower_vehicles_to(self, vehicle, count=2):
        sorted_v = sorted([v for v in self.vehicles
                           if v is not vehicle
                           and v.index >= self.leader_num],
                          key=lambda v: abs(vehicle.lane_distance_to(v)))
