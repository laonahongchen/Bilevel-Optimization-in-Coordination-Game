...
from bilevel_pg.bilevelpg.trainer.bilevel_trainer_highway_iq import Bilevel_Trainer
from bilevel_pg.bilevelpg.utils.random import set_seed
from bilevel_pg.bilevelpg.logger.utils import set_logger
from bilevel_pg.bilevelpg.samplers.sampler_highway_iq import MASampler
...
for seed in range(135, 136):
    ...
    train_agents.append(get_independent_q_agent(env, 1, hidden_layer_sizes=hidden_layer_sizes, max_replay_buffer_size=max_replay_buffer_size))
    ...
    '''
    restore_path = './models/agents_iq_1x1_'+'seed'+str(seed)+'.pickle'
    trainer.restore(restore_path)
    sampler.train_agents = trainer.train_agents
    '''
    trainer.run()
    trainer.save()
    #3np.save("./curves/rewards_i_seed35.npy", sampler.rewards_record)