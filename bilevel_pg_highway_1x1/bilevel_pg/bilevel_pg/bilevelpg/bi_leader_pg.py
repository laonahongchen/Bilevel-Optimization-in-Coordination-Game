...
class LeaderAgent(OffPolicyAgent):
    ...
    def critic_loss(self,
        ...
        target_critic_input = np.hstack((next_observations.astype(np.float32)[:,0].reshape(-1,1),
                                         tf.one_hot(target_actions[:,0], self.action_space.n), tf.one_hot(target_actions[:,1], self.action_space.n)))
        ...
        critic_net_input = np.hstack((observations[:,0].reshape(-1,1), actions, opponent_actions))
        # print('shape compare')
        # print(actions.shape)
        # print(actions[0])
        ...
        print(np.hstack((critic_net_input, rewards.reshape(-1,1), target_q_values)))
        ...
        print("-----------Q values for----------", actions, "-----", q_values)
        print("-----------LOSS------------", critic_loss)
        #with tf.GradientTape(watch_accessed_variables=False) as tape:
           # print(tape.gradient(critic_loss, critic_net_input))
    ...
    def actor_loss(self, observations, opponent_actions, weights=None):
        ...
        for action in range(policies.shape[1]):
            ...
            q_values = self._qf.get_values(tf.concat((tf.reshape(observations.astype(np.float32)[:,0], shape=[tf.shape(observations)[0],1]), actions, opponent_actions), 1))
            # print(type(q_values))
            # if weights is not None:
