...
class LeaderAgent(OffPolicyAgent):
    ...
    def critic_loss(self,
        ...
        #print(target_actions.shape)   #(32, agent_num)
        action_num = env.action_num
        level_agent_num = env.level_agent_num
        num_state = env.num_state
        
        #print(next_observations.shape)   #(32, num_state)
        ...
        #ego_vehicle = env.road.vehicles[agent_id]
        #closest_leaders = env.road.closest_leader_vehicles_to(ego_vehicle, level_agent_num)
        #print(tf.one_hot(target_actions[:, agent_id], action_num))
        #print(actions.shape)
        #print(opponent_actions[:, 1 - agent_id])
        ...
        #print(tf.one_hot(opponent_actions[:, 1 - agent_id], action_num))
        #print(next_obs_v_idxes.shape)
        #print(target_actions.shape, opponent_actions.shape)
        #print(opponent_actions.shape)
        #print(tf.one_hot(target_actions[:, 1 - agent_id], action_num))
        #print(tf.one_hot(opponent_actions[:, 1 - agent_id], action_num))
        ...
        for i in range(batch_size):
            #print(target_actions[i, next_obs_v_idxes[i]])
            #print(target_actions[i])
            #print(next_obs_v_idxes[i])
            #print(tf.one_hot(target_actions[i, next_obs_v_idxes[i]], action_num))
            #print(np.array(tf.one_hot(target_actions[i, next_obs_v_idxes[i]], action_num)).reshape(1, -1)[0])
            target_actions_concat[i, action_num:] = np.array(tf.one_hot(target_actions[i, next_obs_v_idxes[i]], action_num)).reshape(1, -1)[0]
            actions_concat[i, action_num:] = np.array(tf.one_hot(opponent_actions[i, obs_v_idxes[i]], action_num)).reshape(1, -1)[0]
        '''
    ...
    def actor_loss(self, env, agent_id, observations, opponent_actions, weights=None):
        ...
        #actions_concat[:env.action_num] = tf.one_hot(opponent_actions[:, agent_id], self.action_space.n)
        #print(opponent_actions.shape)
        opponent_actions = np.int32(opponent_actions)
        actions_concat[:, env.action_num:] = tf.one_hot(opponent_actions[:, 1 - agent_id], env.action_num)
        '''
        for i in range(batch_size):
            actions_concat[i, env.action_num:] = np.array(tf.one_hot(opponent_actions[i, obs_v_idxes[i]], env.action_num)).reshape(1, -1)[0]
        '''
