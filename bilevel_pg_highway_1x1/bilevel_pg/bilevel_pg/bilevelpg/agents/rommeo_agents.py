...
class ROMMEOAgent(OffPolicyAgent):
    def __init__(self,
                 ...
                 replay_buffer,
                 opponent_policy,
                 policy_optimizer=tf.optimizers.Adam(10e-3),
                 qf_optimizer=tf.optimizers.Adam(10e-3),
                 opponent_policy_optimizer=tf.optimizers.Adam(10e-3),
                 opponent_prior_optimizer=tf.optimizers.Adam(10e-3),
    ...
    def act(self, observation, step=None, use_target=False):
        ...
        if not self._exploration_status:
            with policy.set_deterministic(True):
                action = policy.get_action_np(policy_input, extend_dim=False)
            return action
    ...
    def opponent_policy_loss(self,
        ...
        opponent_actions_log_pis = self._opponent_policy.log_pis([observations], opponent_actions)
        ...
        actions_log_pis = self._policy.log_pis([observations, opponent_actions], actions)

        q_values = self._qf.get_values([observations, actions, opponent_actions])

        opponent_policy_loss = tf.reduce_mean(opponent_actions_log_pis - prior_log_pis -
    ...
    def critic_loss(self,
        ...
        next_opponent_actions = self._opponent_policy.get_actions(next_observations)
        next_opponent_actions_log_pis = self._opponent_policy.log_pis([next_observations], next_opponent_actions)

        next_prior_log_pis = self._prior.log_pis([next_observations], next_opponent_actions)

        next_actions = self._policy.get_actions([next_observations, next_opponent_actions])
        next_actions_log_pis = self._policy.log_pis([next_observations, next_opponent_actions], actions)
    ...
    def actor_loss(self, observations, annealing, weights=None):
        ...
        actor_loss = tf.reduce_mean(annealing*actions_log_pis - tf.reduce_mean(q_values))
        return actor_loss
