...
class Bilevel_Trainer:
    ...
    def run(self):
        ...
        for step in range(self.steps):
            ...
            if self.env.merge_count % 100 == 0:
                np.save('./curves/reward0_MADDPG_1x1_test'+str(self.seed)+'_t2.npy', self.env.episodes_reward_0)
                np.save('./curves/reward1_MADDPG_1x1_test'+str(self.seed)+'_t2.npy', self.env.episodes_reward_1) 
                np.save('./curves/success_merge_MADDPG_1x1_test'+str(self.seed)+'_t2.npy', self.env.episode_merge_record)
                np.save('./curves/target_merge_MADDPG_1x1_test'+str(self.seed)+'_t2.npy', self.env.episode_target_merge_record) 
    ...
    def save(self):
        save_path = './models/agents_maddpg_1x1_'+'test'+str(self.seed)+'.pickle'
