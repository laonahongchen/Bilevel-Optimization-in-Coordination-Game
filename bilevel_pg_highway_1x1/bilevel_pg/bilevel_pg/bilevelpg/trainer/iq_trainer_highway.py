...
class IQ_Trainer:
    ...
    def run(self):
        ...
        for step in range(self.steps):
            ...
            if self.env.merge_count % 100 == 0:
                np.save('./curves/reward0_IQ_1x1_'+str(self.seed)+'npy', self.env.episodes_reward_0)
                np.save('./curves/reward1_IQ_1x1_'+str(self.seed)+'.npy', self.env.episodes_reward_1) 
                np.save('./curves/success_merge_IQ_1x1_'+str(self.seed)+'.npy', self.env.episode_merge_record)
                np.save('./curves/target_merge_IQ_1x1_'+str(self.seed)+'_.npy', self.env.episode_target_merge_record)   
            '''
    ...
    def save(self):
        if self.save_path is None:
            self.save_path = './models/agents_iq_1x1_'+''+str(self.seed)+'.pickle'
