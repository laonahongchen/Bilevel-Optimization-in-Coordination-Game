...
class IndexedReplayBuffer(Serializable):
    def __init__(self, observation_dim, action_dim, next_observation_dim, opponent_action_dim=None, max_replay_buffer_size=10e4):
        ...
        #self._Q_actions = np.zeros((self._max_buffer_size, self._action_dim * (1 + 2 * self.level_v_num)))
        #self._obs_v_idxes = np.zeros((self._max_buffer_size, 2 * self.level_v_num))
        #self._next_obs_v_idxes = np.zeros((self._max_buffer_size, 2 * self.level_v_num))
        #self._pres_valid_conditions = np.zeros((self._max_buffer_size, 1))
        #self._next_valid_conditions = np.zeros((self._max_buffer_size, 1))
    ...
    def add_sample(self, observation, action, reward, terminal,
        ...
        # print(action.shape)
        #self._vehicle_index[self._top] = vehicle_index
        #self._leader_indexes[self._top] = leader_indexes
        self._observations[self._top] = np.squeeze(observation)
        self._actions[self._top] = action
        self._rewards[self._top] = reward
        self._terminals[self._top] = terminal
        self._next_obs[self._top] = next_observation
        #self._Q_actions[self._top] = Q_actions
        #self._obs_v_idxes[self._top] = obs_v_idxes
        #self._next_obs_v_idxes[self._top] = next_obs_v_idxes
        #self._pres_valid_conditions[self._top][0] = pres_valid_conditions
        #self._next_valid_conditions[self._top][0] = next_valid_conditions
        #print(self._top, self._opponent_actions[self._top])
    ...
    def batch_by_indices(self, indices):
        batch = dict(
            ...
            #Q_actions=self._Q_actions[indices].astype(np.float32),
            #obs_v_idxes=self._obs_v_idxes[indices].astype(np.int16),
            #next_obs_v_idxes=self._next_obs_v_idxes[indices].astype(np.int16),
        )
